...
namespace Rock.Blocks.Group.Scheduling
{
    ...
    [LinkedPage( "Decline Reason Page",
        ...
        Description = @"If the group type has enabled ""Requires Reason If Schedule Declined"" then specify the page to provide that reason here.",
...
<div class=""alert alert-info"">
    ...
    {% endif %}
            ...
            return rows
                ...
                .Select( r =>
                {
                    ...
                    return new ScheduleRowBag
                    {
                        ...
                        ScheduleName = r.Schedule?.Name, // TODO (Jason): Should this have a fallback from Name?
        ...
        {
            errorMessage = null;
            ToolboxScheduleRowConfirmationStatus? newStatus = null;
            ...
            if ( !toolboxData.CurrentScheduleRows.Any( r => r.EntityGuid == bag.EntityGuid ) )
            {
                ...
                {
                    return new ScheduleRowActionResponseBag();
                }
            ...
            else
            {
                ...
                if ( bag.ActionType == ToolboxScheduleRowActionType.Accept )
                {
                    ...
                    newStatus = ToolboxScheduleRowConfirmationStatus.Confirmed;
                }
                else if ( bag.ActionType == ToolboxScheduleRowActionType.Decline )
                {
                    if ( attendance.Occurrence?.Group?.GroupType?.RequiresReasonIfDeclineSchedule == true )
                    {
                        // Redirect to the declined reasons page. That page/block will dictate whether to send emails.
                        var queryParams = new Dictionary<string, string>
                        {
                            { PageParameterKey.AttendanceId, attendance.Id.ToString() },
                            { PageParameterKey.IsConfirmed, "false" },
                            { PageParameterKey.ReturnUrl, this.PageCache.Guid.ToString() }
                        };

                        var declinedReasonsPageUrl = this.GetLinkedPageUrl( AttributeKey.DeclineReasonPage, queryParams );
                        if ( declinedReasonsPageUrl.IsNullOrWhiteSpace() )
                        {
                            ExceptionLogService.LogException( $@"{nameof( GroupScheduleToolbox )}: When declining schedule, unable to get linked Page URL for ""Decline Reasons Page"" (Attendance ID = {attendance.Id}, Scheduled Person ID = {toolboxData.SelectedPerson.Id})." );
                        ...
                        return new ScheduleRowActionResponseBag
                        {
                            DeclinedReasonsPageUrl = declinedReasonsPageUrl
                            // New status doesn't matter here, as the UI will simply redirect to the declined reasons page.
                        };
                    }
                    else
                    {
                        ...
                        // Don't set a new status, so this schedule row will be removed.
                    }
                }
                else // ToolboxScheduleRowActionType.Cancel
                {
                    ...
                    newStatus = ToolboxScheduleRowConfirmationStatus.Pending;
                }
